...
import { NEWS_SEARCH_URL } from "@/lib/news";
...
function SkeletonCard() {
  return (
    <div className="card card-hover flex h-full flex-col gap-4 p-4">
...
function NewsCard({ article }: { article: NewsArticle }) {
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="card card-hover flex h-full flex-col gap-4 p-4 transition-transform duration-150 hover:-translate-y-0.5"
    >
      <div className="flex items-center justify-between gap-3">
...
export function LatestNewsSection() {
    ...
    fetch("/api/news", { signal: controller.signal })
      .then(async (response) => {
        ...
        if (nextItems.length === 0) {
          setError("No news items matched the current query.");
        }
      })
      .catch((err: unknown) => {
  ...
  return (
    <section className="mt-12 rounded-[var(--radius-xl)] border border-[var(--color-border)] bg-[rgba(9,15,14,0.9)] p-4 md:p-5">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h2 className="mt-2 text-[20px] font-bold tracking-[-0.02em] text-[var(--color-text-primary)]">
            What the Monad ecosystem is talking about
          </h2>
          <p className="mt-1 max-w-[760px] text-[12px] leading-relaxed text-[var(--color-text-muted)]">
            Live headlines from Google News searches around Monad, DeFi, and the
            wider ecosystem. We cache the feed so the section stays fast.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {meta?.status === "stale" && (
            <span className="rounded-full border border-[rgba(255,214,76,0.25)] bg-[rgba(255,214,76,0.08)] px-2.5 py-1 text-[11px] font-semibold text-[var(--color-warning)]">
              Cached fallback
            </span>
          )}
          <button
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="mt-4 flex items-center justify-between gap-3 text-[11px] text-[var(--color-text-dim)]">
            <span>
              Sourced from Google News searches for Monad, DeFi, and ecosystem
              coverage.
            </span>
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="font-semibold text-[var(--color-accent-primary)] hover:opacity-80"
            >
              Open full feed
            </a>
          </div>
        ...
        <div className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[rgba(255,255,255,0.025)] px-4 py-5">
          <div className="text-[14px] font-semibold text-[var(--color-text-primary)]">
            {error || "No headlines are available right now."}
          </div>
          <p className="mt-1 text-[12px] leading-relaxed text-[var(--color-text-muted)]">
            The feed can be temporarily empty if the source search returns no
            results or the upstream news service is rate-limited. Try again or
            open the full search feed.
          </p>
          <a
            href={NEWS_SEARCH_URL}
            target="_blank"
            rel="noreferrer"
            className="mt-3 inline-flex rounded-[var(--radius-md)] bg-[var(--color-accent-primary)] px-3 py-2 text-[11px] font-bold text-[#07110C]"
          >
            Open news search
          </a>
        </div>
